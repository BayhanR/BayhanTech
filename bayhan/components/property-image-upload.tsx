...
export function PropertyImageUpload({ propertyId, userId, onSuccess }: PropertyImageUploadProps) {
    ...
    const fetchImages = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setError("Bu emlaka erişim yetkiniz yok")
        }
      } catch (err) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        formData.append('itemId', propertyId)
  ...
  return (
    <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
      <CardHeader>
        <CardTitle>Emlak Fotoğrafları</CardTitle>
        <CardDescription>Emlak fotoğraflarınızı yükleyin</CardDescription>
      </CardHeader>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {images.map((image) => (
              <div key={image} className="relative group rounded-lg overflow-hidden">
                <img src={image || "/placeholder.svg"} alt="Emlak" className="w-full h-32 object-cover" />
                <button
